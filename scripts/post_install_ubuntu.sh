...
sudo apt update
sudo apt upgrade
sudo apt install -y git g++ python3 ubuntu-restricted-extras python3-pip vim snapd ibus fzf zsh xclip curl fonts-hack
...
sudo apt install -y ~/Downloads/wps-office_11.1.0.9615.XA_amd64.deb
...
sudo apt install -y ~/Downloads/skypeforlinux-64.deb
...
sudo apt install -y ~/Downloads/vscode-amd64.deb
